...
class MessageStream {
  ...
  private waiting: ((value: IteratorResult<Message>) => void)[] = [];
...
async function startupScan(stream: MessageStream, approvalLabel: string): Promise<void> {
...
async function main(): Promise<void> {
  ...
  if (options.mode === 'polling') {
    startPoller(
      stream,
      config.automation.approvalLabel,
