...
type ParseColumn<Col extends string> = ReadIdentifier<Trim<Col>> extends [
...
type ValidateColumnRefs<List extends string, Names extends string> = Trim<List> extends ""
	? true
...
type ParseCreateBody<S extends string, Row, Names extends string, Error = never> = Trim<S> extends ""
	...
	? IsConstraintEntry<Head> extends true
		? ParseCreateBody<Tail, Row, Names, MergeError<Error, ValidateConstraintRefs<Head, Names>>>
		: AddColumn<Head, Row, Names> extends infer Next extends {
				row: unknown
				names: string
				error: unknown
		  }
		? ParseCreateBody<Tail, Next["row"], Next["names"], MergeError<Error, Next["error"]>>
		: {
				row: Row
				names: Names
 ...
 * Also supports:
 * - quoted identifiers
 * - parsing table constraints for validation without affecting row shape
 */
...
> = Tables extends readonly [infer Head extends SqlCreateTableLike, ...infer Tail extends readonly SqlCreateTableLike[]]
	? Head["name"] extends infer Name
		? Name extends SqlParseError<string>
			? SqlDatabaseInternal<Tail, Acc, Seen, Error | Name>
			: Name extends string
			? Name extends Seen
				? SqlDatabaseInternal<Tail, Acc, Seen, Error | SqlParseError<`Duplicate table name: ${Name}`>>
				: Head["row"] extends infer Row
				? Row extends SqlParseError<string>
					? SqlDatabaseInternal<Tail, Acc, Seen | Name, Error | Row>
					: SqlDatabaseInternal<Tail, Acc & { [K in Name]: Row }, Seen | Name, Error>
				: SqlDatabaseInternal<Tail, Acc, Seen | Name, Error | SqlParseError<"Internal SQL parser error">>
			: SqlDatabaseInternal<
					Tail,
					Acc,
					Seen,
			  ...
			  >
		: SqlDatabaseInternal<Tail, Acc, Seen, Error | SqlParseError<"Expected a CREATE TABLE statement with a table name">>
	: [Error] extends [never]
	? Simplify<Acc>
	: Error

export type SqlDatabase<Tables extends readonly SqlCreateTableLike[]> = SqlDatabaseInternal<Tables, {}, never>
