...
type StripConstraintPrefix<S extends string> =
	ToLower<Trim<S>> extends `constraint ${infer Rest}`
		? ReadIdentifier<Trim<Rest>> extends [string, infer AfterName extends string]
			? Trim<ToLower<AfterName>>
			: ToLower<Trim<S>>
		: ToLower<Trim<S>>
export type IsConstraintEntry<S extends string> =
	StripConstraintPrefix<S> extends
		| `primary key${string}`
		| `unique${string}`
		| `foreign key${string}`
		| `check${string}`
...
export type ValidateColumnRefs<List extends string, Names extends string> =
	Trim<List> extends ""
		? true
...
export type ParseColumnListToTuple<List extends string> =
	Trim<List> extends ""
		? readonly []
		: ReadUntilTopLevelComma<List> extends [infer Head extends string, infer Tail extends string]
...
export type ValidateConstraintRefs<Entry extends string, Names extends string> =
...
export type ParseForeignRefMeta<Entry extends string> =
	StripConstraintPrefix<Entry> extends infer E extends string
		? E extends `foreign key${string}`
