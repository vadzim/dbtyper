...
type _XApplyArgAcceptsParsedString = Expect<
	X extends {
		apply(statement: ReturnType<typeof sqlStatement<"alter table users add column age int">>): unknown
	}
		? true
		: false
>
...
type _X4MigrationsGetter = Expect<
	Equal<ReturnType<X4["getMigrations"]>, Promise<{ source: string; path: string }[]>>
>
