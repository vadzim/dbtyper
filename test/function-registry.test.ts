...
type _customFn = Expect<Extends<Tuple3At2<TCustomFn>, { kind: "select"; columns: { "?column?": TInteger } }>>
type TCountStar = ParseSqlStatement<ParseSqlTokens<`select count(*) from t`>, DbFns>
type _countStar = Expect<Extends<Tuple3At2<TCountStar>, { kind: "select"; columns: { "?column?": TBigint } }>>
type TCountOne = ParseSqlStatement<ParseSqlTokens<`select count(1) from t`>, DbFns>
type _countOne = Expect<Extends<Tuple3At2<TCountOne>, { kind: "select"; columns: { "?column?": TBigint } }>>
type TUuidV4 = ParseSqlStatement<ParseSqlTokens<`select uuid_generate_v4() from t`>, DbFns>
type _uuidV4 = Expect<Extends<Tuple3At2<TUuidV4>, { kind: "select"; columns: { "?column?": TUuid } }>>
type TLower = ParseSqlStatement<ParseSqlTokens<`select lower('a') from t`>, DbFns>
type _lower = Expect<Extends<Tuple3At2<TLower>, { kind: "select"; columns: { "?column?": TText } }>>
/** `coalesce` picks first argument type. */
type TCoalesce = ParseSqlStatement<ParseSqlTokens<`select coalesce(x, x) from t`>, DbFns>
type _coalesce = Expect<Extends<Tuple3At2<TCoalesce>, { kind: "select"; columns: { "?column?": TInteger } }>>
...
type _sum = Expect<Extends<Tuple3At2<TSum>, { kind: "select"; columns: { "?column?": TNumeric } }>>
...
type _now = Expect<Extends<Tuple3At2<TNow>, { kind: "select"; columns: { "?column?": TTimestamp } }>>
type TGenRandomUuid = ParseSqlStatement<ParseSqlTokens<`select gen_random_uuid() from t`>, DbFns>
type _uuid = Expect<Extends<Tuple3At2<TGenRandomUuid>, { kind: "select"; columns: { "?column?": TUuid } }>>
...
type _upperNum = Expect<Extends<Tuple3At2<TUpperNum>, { kind: "select"; columns: { "?column?": TText } }>>
type TUpper = ParseSqlStatement<ParseSqlTokens<`select upper('x') from t`>, DbFns>
type _upper = Expect<Extends<Tuple3At2<TUpper>, { kind: "select"; columns: { "?column?": TText } }>>
