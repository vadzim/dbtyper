...
// ❌ ERROR: Correlated subquery referencing outer table in inner select list
const query = `select u.id from users as u join (select u.id from users) t on u.id = t.id;` as const
