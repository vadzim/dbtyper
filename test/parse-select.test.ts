...
type TSelectCaseKw = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _selectCaseKw = Expect<Extends<TSelectCaseKw[2], { kind: "select"; returning: { x: TUuid } }>>
...
type TSelectCaseSimple = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _selectCaseSimple = Expect<Extends<TSelectCaseSimple[2], { kind: "select"; returning: { x: TText } }>>
...
type _selectCaseSimpleNoElse = Expect<
	Extends<TSelectCaseSimpleNoElse[2], { kind: "select"; returning: { x: TNull<"text"> } }>
>
...
type TWithCte = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _withCte = Expect<Extends<TWithCte[2], { kind: "select"; returning: { uid: TUuid } }>>
...
type TDerivedTable = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _derivedTable = Expect<Extends<TDerivedTable[2], { kind: "select"; returning: { id: TUuid } }>>
...
type TDerivedJoin = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _derivedJoin = Expect<Extends<TDerivedJoin[2], { kind: "select"; returning: { id: TUuid } }>>
...
type TDerivedInnerWhere = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _derivedInnerWhere = Expect<Extends<TDerivedInnerWhere[2], { kind: "select"; returning: { id: TUuid } }>>

type TDerivedInnerDistinct = ParseSqlStatement<
	CreateParserMonad<`select s.id from (select distinct users.id from users) as s;`>,
	DbJoinDefaultAndExplicit
>
type _derivedInnerDistinct = Expect<Extends<TDerivedInnerDistinct[2], { kind: "select"; returning: { id: TUuid } }>>
...
type TDerivedInnerJoin = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _derivedInnerJoin = Expect<Extends<TDerivedInnerJoin[2], { kind: "select"; returning: { id: TUuid } }>>
...
type _derivedLeftOuterJoinRhs = Expect<
	Extends<TDerivedLeftOuterJoinRhs[2], { kind: "select"; returning: { id: TUuid } }>
>
...
type _derivedInnerParam = Expect<Extends<_derivedInnerParamActual, { kind: "select"; returning: { id: TUuid } }>>
...
type _selectStar = Expect<Extends<TSelectStar[2], { kind: "select"; returning: { id: TUuid; name: TText } }>>
...
type _catalogQualColumnSelect = Expect<
	Extends<TCatalogQualColumnSelect[2], { kind: "select"; returning: { id: TUuid } }>
>

type TAsAlias = ParseSqlStatement<CreateParserMonad<`select users.id as uid from users;`>, DbJoinDefaultAndExplicit>
type _asAlias = Expect<Extends<TAsAlias[2], { kind: "select"; returning: { uid: TUuid } }>>

type TScalarAdd = ParseSqlStatement<CreateParserMonad<`select 1 + 1 as a from users;`>, DbJoinDefaultAndExplicit>
type _scalarAdd = Expect<Extends<TScalarAdd[2], { kind: "select"; returning: { a: TInteger } }>>
...
type TColPlusOne = ParseSqlStatement<
	...
	DbJoinWithRowCount
>
type _colPlusOne = Expect<Extends<TColPlusOne[2], { kind: "select"; returning: { x: TInteger } }>>
...
type TColPlusOneNoAs = ParseSqlStatement<
	...
	DbJoinWithRowCount
>
type _colPlusOneNoAs = Expect<Extends<TColPlusOneNoAs[2], { kind: "select"; returning: { "?column?": TInteger } }>>

type TSelectLiteralOne = ParseSqlStatement<CreateParserMonad<`select 1 from users;`>, DbJoinWithRowCount>
type _selectLiteralOne = Expect<Extends<TSelectLiteralOne[2], { kind: "select"; returning: { "?column?": TInteger } }>>
...
type _selectParam = Expect<
	Extends<
		...
		{
			kind: "select"
			returning: { limit: TInteger; id: TUuid }
		}
	>
>
...
type _selectParamAs = Expect<
	Extends<
		...
		{
			kind: "select"
			returning: { page_size: TInteger; name: TText }
		}
	>
>
...
type TUnqualNameUnambiguous = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _unqualNameOk = Expect<Extends<TUnqualNameUnambiguous[2], { kind: "select"; returning: { name: TText } }>>
...
type TSelectBoolCmpAnd = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _selectBoolCmpAnd = Expect<Extends<TSelectBoolCmpAnd[2], { kind: "select"; returning: { ok: TBoolean } }>>

type TSelectBoolOrAndPrec = ParseSqlStatement<
	CreateParserMonad<`select true or false and false as p from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectBoolOrAndPrec = Expect<Extends<TSelectBoolOrAndPrec[2], { kind: "select"; returning: { p: TBoolean } }>>

type TSelectNotFalse = ParseSqlStatement<
	CreateParserMonad<`select not false as x from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectNotFalse = Expect<Extends<TSelectNotFalse[2], { kind: "select"; returning: { x: TBoolean } }>>

type TSelectCmpAdd = ParseSqlStatement<
	CreateParserMonad<`select 1 + 2 > 2 as gt from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectCmpAdd = Expect<Extends<TSelectCmpAdd[2], { kind: "select"; returning: { gt: TBoolean } }>>

type TSelectIsNull = ParseSqlStatement<
	CreateParserMonad<`select (users.name is null) as n from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectIsNull = Expect<Extends<TSelectIsNull[2], { kind: "select"; returning: { n: TBoolean } }>>
...
type TSelectInList = ParseSqlStatement<
	...
	DbJoinDefaultAndExplicit
>
type _selectInList = Expect<Extends<TSelectInList[2], { kind: "select"; returning: { inside: TBoolean } }>>

type TSelectPgCast = ParseSqlStatement<
	CreateParserMonad<`select 42::text as t, (1 + 2)::bigint as b from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectPgCast = Expect<Extends<TSelectPgCast[2], { kind: "select"; returning: { t: TText; b: TBigint } }>>

type TSelectSqlCast = ParseSqlStatement<
	CreateParserMonad<`select cast(true as text) as flag_txt from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectSqlCast = Expect<Extends<TSelectSqlCast[2], { kind: "select"; returning: { flag_txt: TText } }>>

type TSelectCastIntErr = ParseSqlStatement<
	CreateParserMonad<`select cast('x' as integer) as bad from users;`>,
	DbJoinDefaultAndExplicit
>
type _selectCastIntErr = Expect<Extends<TSelectCastIntErr[2], { kind: "select"; returning: { bad: TInteger } }>>
...
`>,
	DbJoinDefaultAndExplicit
>
type _withTwoCtes = Expect<Extends<TWithTwoCtes[2], { kind: "select"; returning: { id: TUuid } }>>
