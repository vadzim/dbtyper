...
type W1 = ParseWhereExpression<CreateParserMonad<`users.id = 'u'`>, DbUsers, UsersScope>
type _w1 = Expect<Extends<W1[1], null>>

type W3part = ParseWhereExpression<CreateParserMonad<`public.users.id = 'u'`>, DbUsers, UsersScope>
...
type WTwoLayer = ParseWhereExpression<CreateParserMonad<`outer_t.b = 'x'`>, DbUsers, JoinedOuterInner>
type _wTwoLayer = Expect<Extends<WTwoLayer[1], null>>

type WAnd = ParseWhereExpression<CreateParserMonad<`users.id = 'u' and users.name is null`>, DbUsers, UsersScope>
type _wAnd = Expect<Extends<WAnd[1], null>>

/** PostgreSQL `~`: regex match on text columns. */
type WRegex = ParseWhereExpression<CreateParserMonad<`users.name ~ 'a'`>, DbUsers, UsersScope>
type _wRegex = Expect<Extends<WRegex[1], null>>

/** Success: comparison operators and literals. */
type WNe = ParseWhereExpression<CreateParserMonad<`users.id <> 'v'`>, DbUsers, UsersScope>
type _wNe = Expect<Extends<WNe[1], null>>
type WNeBang = ParseWhereExpression<CreateParserMonad<`users.id != 'v'`>, DbUsers, UsersScope>
type _wNeBang = Expect<Extends<WNeBang[1], null>>
type WLe = ParseWhereExpression<CreateParserMonad<`users.name <= 'z'`>, DbUsers, UsersScope>
type _wLe = Expect<Extends<WLe[1], null>>
type WStr = ParseWhereExpression<CreateParserMonad<`users.name = 'a'`>, DbUsers, UsersScope>
type _wStr = Expect<Extends<WStr[1], null>>

/** Success: `IS NOT NULL`, `IN (...)`, nested parens, `OR` inside parens. */
type WIsNotNull = ParseWhereExpression<CreateParserMonad<`users.name is not null`>, DbUsers, UsersScope>
type _wIsNotNull = Expect<Extends<WIsNotNull[1], null>>
type WInList = ParseWhereExpression<CreateParserMonad<`users.id in ( 'a' , 'b' )`>, DbUsers, UsersScope>
type _wInList = Expect<Extends<WInList[1], null>>

type WNested = ParseWhereExpression<CreateParserMonad<`( ( users.id = 'u' ) )`>, DbUsers, UsersScope>
type _wNested = Expect<Extends<WNested[1], null>>
type WOrInParens = ParseWhereExpression<
	CreateParserMonad<`users.id = 'u' and ( users.name = 'a' or users.name = 'b' )`>,
	DbUsers,
	UsersScope
>
type _wOrInParens = Expect<Extends<WOrInParens[1], null>>

type WNotNot = ParseWhereExpression<CreateParserMonad<`not not users.id = 'u'`>, DbUsers, UsersScope>
type _wNotNot = Expect<Extends<WNotNot[1], null>>

/** `BETWEEN`: bounds same comparison class as subject. */
type WBetween = ParseWhereExpression<CreateParserMonad<`users.name between 'a' and 'z'`>, DbUsers, UsersScope>
...
type WLike = ParseWhereExpression<CreateParserMonad<`users.name like '%x%'`>, DbUsers, UsersScope>
type _wLike = Expect<Extends<WLike[1], null>>
type WILike = ParseWhereExpression<CreateParserMonad<`users.name ilike '%X%'`>, DbUsers, UsersScope>
...
type WCase = ParseWhereExpression<
	CreateParserMonad<`case when users.name = 'a' then true else false end`>,
	DbUsers,
	UsersScope
>
...
type WCaseSimple = ParseWhereExpression<
	CreateParserMonad<`case users.id when 'u' then true else false end`>,
	DbUsers,
	UsersScope
>
...
type WCaseSimpleNoElse = ParseWhereExpression<
	CreateParserMonad<`case users.id when 'u' then true end`>,
	DbUsers,
	UsersScope
>
...
type WCastTextToInteger = ParseWhereExpression<
	CreateParserMonad<`cast(users.name as integer) = 1`>,
	DbUsers,
	UsersScope
>
 ...
 * `CreateParserMonad` widens bare identifiers to `string`, so full-parser ambiguity is not asserted here;
