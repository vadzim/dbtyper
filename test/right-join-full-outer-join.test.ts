...
type TRightJoin = ParseSqlStatement<
	CreateParserMonad<`select * from users right join orders on users.id = orders.user_id;`>,
	DbJoins
>
...
type TRightOuterJoin = ParseSqlStatement<
	CreateParserMonad<`select * from users right outer join orders on users.id = orders.user_id;`>,
	DbJoins
>
...
type TFullOuterJoin = ParseSqlStatement<
	CreateParserMonad<`select * from users full outer join orders on users.id = orders.user_id;`>,
	DbJoins
>
...
type TFullJoin = ParseSqlStatement<
	CreateParserMonad<`select * from users full join orders on users.id = orders.user_id;`>,
	DbJoins
>
