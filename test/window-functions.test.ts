...
type TRowNumber = ParseSqlStatement<
	CreateParserMonad<`select id, product, row_number() over (order by amount) as row_num from sales;`>,
	DbWindow
>
...
type TRowNumberDesc = ParseSqlStatement<
	CreateParserMonad<`select id, product, row_number() over (order by amount desc) as row_num from sales;`>,
	DbWindow
>
...
type TRank = ParseSqlStatement<
	CreateParserMonad<`select id, product, rank() over (order by amount) as rank_num from sales;`>,
	DbWindow
>
type _tRank = Expect<
...
type TDenseRank = ParseSqlStatement<
	CreateParserMonad<`select id, product, dense_rank() over (order by amount) as dense_rank_num from sales;`>,
	DbWindow
>
...
type TMultipleWindow = ParseSqlStatement<
	CreateParserMonad<`select id, row_number() over (order by amount) as row_num, rank() over (order by amount) as rank_num from sales;`>,
	DbWindow
>
...
type TPartitionBy = ParseSqlStatement<
	CreateParserMonad<`select id, product, row_number() over (partition by product order by amount) as row_num from sales;`>,
	DbWindow
>
...
type TPartitionByMultiOrder = ParseSqlStatement<
	CreateParserMonad<`select id, rank() over (partition by product order by amount desc, sale_date) as rank_num from sales;`>,
	DbWindow
>
...
type TMultiPartition = ParseSqlStatement<
	CreateParserMonad<`select id, row_number() over (partition by product, sale_date order by amount) as row_num from sales;`>,
	DbWindow
>
...
type TLag = ParseSqlStatement<
	CreateParserMonad<`select id, product, amount, lag(amount) over (partition by product order by sale_date) as prev_amount from sales;`>,
	DbWindow
>
...
type TLead = ParseSqlStatement<
	CreateParserMonad<`select id, product, amount, lead(amount) over (partition by product order by sale_date) as next_amount from sales;`>,
	DbWindow
>
...
type TLagOffset = ParseSqlStatement<
	CreateParserMonad<`select id, lag(amount, 2) over (order by sale_date) as prev_amount from sales;`>,
	DbWindow
>
...
type TLeadDefault = ParseSqlStatement<
	CreateParserMonad<`select id, lead(amount, 1, 0) over (order by sale_date) as next_amount from sales;`>,
	DbWindow
>
